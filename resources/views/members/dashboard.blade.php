...
<section class="mb-8">
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm overflow-hidden">
        <div class="flex items-start justify-between mb-4">
            <div>
                {{-- Menampilkan Nama Paket dari relasi membership --}}
                <h3 class="text-xl font-semibold mb-2 text-gray-900">
                    {{ $member->membership ? $member->membership->nama_plan : 'No Active Membership' }}
                </h3>
        ...
        @if($member->membership)
            {{-- Logika menghitung tanggal --}}
            @php
                $joinDate = \Carbon\Carbon::parse($member->tanggal_bergabung);
                // Asumsi durasi membership 1 bulan, sesuaikan jika ada field durasi di database
                $expiryDate = $joinDate->copy()->addMonth(); 
        ...
        @else
            <p class="text-gray-500 italic">You have not joined any membership plan yet.</p>
        @endif
