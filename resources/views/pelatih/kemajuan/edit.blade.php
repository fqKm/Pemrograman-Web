            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('progress.update', $progress->id) }}" method="POST">
                        ...
                        {{-- ID Kelas (Hidden) --}}
                        <input type="hidden" name="kelas_id" value="{{ $progress->kelas_id }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="nama_latihan" id="nama_latihan" value="{{ old('nama_latihan', $progress->nama_latihan) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="alat_id" id="alat_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                ...
                                @foreach ($alat as $data_alat)
                                    <option value="{{ $data_alat->id }}" {{ old('alat_id', $progress->alat_id) == $data_alat->id ? 'selected' : '' }}>
                                        {{ $data_alat->nama_alat }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="jumlah_set" id="jumlah_set" value="{{ old('jumlah_set', $progress->jumlah_set) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required min="1">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="jumlah_repetisi" id="jumlah_repetisi" value="{{ old('jumlah_repetisi', $progress->jumlah_repetisi) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required min="1">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="deskripsi" id="deskripsi" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">{{ old('deskripsi', $progress->deskripsi) }}</textarea>
                            @error('deskripsi')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>

                        <div class="flex items-center justify-end mt-6">
                            {{-- Tombol Batal kembali ke Detail Kelas yang sedang diedit --}}
                            <a href="{{ route('kelas.show', $progress->kelas_id) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded mr-2">Batal</a>
