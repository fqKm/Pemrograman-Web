...
<div class="max-w-3xl mx-auto sm:px-6 lg:px-8 py-12">
    <h1 class="text-2xl font-semibold text-gray-800 mb-4 dark:text-white">Edit Data Member</h1>
        ...
        <div class="p-6">
            <form action="{{ route('members.update', $member) }}" method="POST">
                ...
                @method('PUT') <div class="mb-4">
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $member->nama) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                    @error('nama')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div class="mb-4">
                    <label for="email" class="block text-sm font-medium text-gray-700">Alamat Email</label>
                    <input type="email" name="email" id="email" value="{{ old('email', $member->email) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                    @error('email')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div class="mb-4">
                    <label for="nomor_hp" class="block text-sm font-medium text-gray-700">Nomor Telepon</label>
                    <input type="text" name="nomor_hp" id="nomor_hp" value="{{ old('nomor_hp', $member->nomor_hp) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    @error('nomor_hp')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>

                <div class="mb-4">
                    <label for="tanggal_lahir" class="block text-sm font-medium text-gray-700">Tanggal Lahir</label>
                    <input type="date" name="tanggal_lahir" id="tanggal_lahir" value="{{ old('tanggal_lahir', $member->tanggal_lahir) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="tanggal_bergabung" id="tanggal_bergabung" value="{{ old('tanggal_bergabung', $member->tanggal_bergabung) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                ...
                <div class="mb-4">
                    ...
                    <option value="">Pilih Paket</pre>
                    @foreach ($memberships as $paket)
                        <option value="{{ $paket->id }}" {{ old('membership_id', $member->membership_id) == $paket->id ? 'selected' : '' }}>
                        {{ $paket->nama_plan }}
                        </option>
                    @endforeach
                    </select>
